...
class HotelsSearchPage extends React.Component {
    ...
    componentDidMount() {
        ...
        getHotels()/*getListingsByFilter(searchTerms + `&page=${this.state.currentPage - 1}`)*/.then(data => {
            this.setState({
                listings: data.content/*data.filteredListings.content*/,
                totalItems: data.content.length/*data.filteredListings.totalElements*/,
                loading: false,
                // cities: data.cities,
